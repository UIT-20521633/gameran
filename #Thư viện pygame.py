...
show_menu = True
# Bổ sung một biến để theo dõi trạng thái của trò chơi
playing = False
while True :
    ...
    if modechoi == 1:
        if running:
                ...
                clock1.tick(20)#snake_speed fps
    elif modechoi == 2:
        if running:
        # Xử lý điều khiển và logic cho chế độ 2 người chơi
            ...
            # Điều khiển rắn của người chơi 2 trong chế độ chơi 2 người       
                ...
                if player1_x1 >= dis_rong or player1_x1 < 0 or player1_y1 >= dis_cao or player1_y1 < 20 or (player1_x1, player1_y1) in body_list_1[:-1] or \
                        player2_x1 >= dis_rong or player2_x1 < 0 or player2_y1 >= dis_cao or player2_y1 < 20 or (player2_x1, player2_y1) in body_list_2[:-1]:
                            running = False        
                 #Hiển thị điểm chế độ 2 người 
                if running:
                    score_txt=font_score.render(f'Player1 score: {score_choi1}_______________Player2 score: {score_choi2}',True,(65,105,255))#Print score lên màn hình
                    screen.blit(score_txt,(0,0))#position hiển thị
                else:
                    ...
                    score_show2 = 0   
                #Thời gian càng nhỏ thì răn di chuyển càng chậm có thể dùng để tăng độ khó của game 
                clock2.tick(snake_speed2)#snake_speed fps
    pygame.display.update()
